...
with col_main:
    ...
    with btn_skip:
        if st.button("Skip — scan without holdings"):
    ...
    if st.button("🔍 Scan All Nifty 50", type="primary"):
    ...
    with tab_build:
        ...
        if not build_results:
            ...
            st.caption(
                "Buy the **Shares Needed** to complete 1 lot, then re-scan to see "
    ...
    with tab_none:
        ...
        if not none_results:
            ...
            st.caption(
                "No position held. The lot size shows the minimum shares needed to write "
                "covered calls. Ask Claude below which stocks are best to build first."
            )
